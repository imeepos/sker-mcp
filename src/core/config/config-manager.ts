 ...
 */
export const getConfigManager = (logger?: IWinstonLogger) => {
  if (logger) {
...
export class ConfigManagerFactory {
   ...
   */
  static getInstance(name = 'default', logger?: IWinstonLogger): ConfigManager {
    if (!ConfigManagerFactory.instances.has(name)) {
      if (!logger) {
        throw new Error('ConfigManager requires Logger dependencies');
      }
      ConfigManagerFactory.instances.set(name, new ConfigManager(logger));
    }
   ...
   */
  static createInstance(name: string, logger: IWinstonLogger): ConfigManager {
    const instance = new ConfigManager(logger);
